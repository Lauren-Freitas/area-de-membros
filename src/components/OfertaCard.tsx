...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button
      ...
      style={{ borderColor: 'var(--brand)', color: copied ? '#22c55e' : 'var(--brand-text)', backgroundColor: 'var(--brand-bg)' }}
    >
...
export function OfertaCard({ offer }: { offer: Offer }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border-2 p-5" style={{ borderColor: 'var(--brand)', background: 'linear-gradient(135deg, var(--brand-bg) 0%, #fff9ea 100%)' }}>
      {/* Faixa de desconto */}
      {discount && (
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0 pr-10">
          ...
          {product && <p className="text-xs font-medium mt-0.5" style={{ color: 'var(--brand-text)' }}>{product.title}</p>}
