    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          Resultados para <span style={{ color: 'var(--brand)' }}>&ldquo;{query}&rdquo;</span>
        </h1>
      ...
      {totalResults === 0 ? (
        <div className="text-center py-16 bg-card rounded-2xl border border-gray-100 dark:border-[#1e2030]">
          ...
          <Link href="/dashboard" className="inline-block mt-4 text-sm font-semibold" style={{ color: 'var(--brand)' }}>
            Voltar ao dashboard
          </Link>
        </div>
      ) : (
              ...
              <div className="space-y-2">
                {productResults!.map(p => (
                  <Link
                    key={p.id}
                    href={`/produto/${p.id}`}
                    className="flex items-center gap-4 p-4 bg-card rounded-xl border border-gray-100 dark:border-[#1e2030] hover:border-yellow-300 dark:hover:border-yellow-700 transition group"
                  >
                    <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, var(--brand-bg), #ede0c8)' }}>
                      {p.banner_url
                        // eslint-disable-next-line @next/next/no-img-element
                        ? <img src={p.banner_url} alt="" className="w-full h-full object-cover" />
                        : <span style={{ color: 'var(--brand)' }}>{p.content_type === 'video' ? '▶' : '📄'}</span>
                      }
                    </div>
                ...
                {moduleResults!.map(m => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex items-center gap-4 p-4 bg-card rounded-xl border border-gray-100 dark:border-[#1e2030] hover:border-yellow-300 dark:hover:border-yellow-700 transition group"
                    >
                      <div className="w-10 h-10 rounded-lg shrink-0 flex items-center justify-center text-lg" style={{ backgroundColor: 'var(--brand-bg)' }}>
                        📚
                      </div>
                      <div className="flex-1 min-w-0">
